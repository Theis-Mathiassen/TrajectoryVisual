...
PICKLE_HITS = ['RangeQueryHits.pkl', 'SimilarityQueryHits.pkl'] #['RangeQueryHits_geolife.pkl'] 
...
def prepareQueries(config, origRtree, origTrajectories, useGaussian = False, useDataDistribution = False):
    ...
    origRtreeQueriesTraining : QueryWrapper = QueryWrapper(math.ceil(config.numberOfEachQuery * config.trainTestSplit), random=False, useGaussian=useGaussian, avgCoordinateValues=avgCoordinateValues, rtree=origRtree, trajectories=origTrajectories, sigma=stdCoordinatesValue)
    ...
    # print("len(origTrajectories): ", len(origTrajectories))
    # print("len(origTrajectories): ", len(origTrajectories))
    ...
    origRtreeQueriesEvaluation : QueryWrapper = QueryWrapper(math.floor(config.numberOfEachQuery - config.numberOfEachQuery * config.trainTestSplit),random=False, rtree=origRtree, trajectories = origTrajectories)
...
def main(config):
    ...
    origRtreeQueriesTraining, origRtreeQueriesEvaluation = prepareQueries(config, origRtree, origTrajectories, useGaussian=False, useDataDistribution=False)
    ...
    with open(os.path.join(output_dir, "simplifiedTrajectories" + '.pkl'), 'wb') as file:
        pickle.dump(simpTrajectories, file)
        file.close()
...
def gridSearch(config, args):
    ...
    giveQueryScorings(origRtree, origTrajectories, queryWrapper = None, pickleFiles=PICKLE_HITS, config=config, numberToTrain=200)
    for weight in config.weights:
        ...
        for compression_rate in config.compression_rate:
            ...
            logger.info("Saving simplified trajectories.")
            with open(os.path.join(output_dir, "simplifiedTrajectories" + '.pkl'), 'wb') as file:
                pickle.dump(simpTrajectories, file)
                file.close()
